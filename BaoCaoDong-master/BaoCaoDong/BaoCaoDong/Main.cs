    ...
    public partial class Main : Form
    {
        ...
        private void LoadGrid()
        {
            try
            {
                ds1.Tables.Clear();
                ds2.Tables.Clear();
                query = " select " + Check100() + LayTenCot() + " from tblTongHopThongTin " + ShowDieuKien();
        ...
        private void LoadCmbTTHS()
        {
            cmbTTHS.Items.Clear();
            string _query = " select * from tblTuDienTrangThaiHoSo ";
            string[] items = cls._ExecuteReader(_query, "MoTa");
            foreach (string item in items)
            {
                cmbTTHS.Items.Add(item);
            }
        }
        ...
        {
            string _query = " select * from tblColumns where TrangThai = '1' ";
        ...
        private void FormatDataGridView()
        {
            try
            {
                ...
                string _query = " select A.TenBang,A.TenCot,A.TenCotHienThi,A.DoRong,B.Color,C.TenKieu " +
                                   " from tblColumns as A inner join tblQuanLyBang as B on A.TenBang = B.num " +
        ...
        {
            try {//co the toi uu hon o day, khong dung den ds1 nưa.
               
                dataGridView1.DataSource = ds1.Tables[0];
                FormatDataGridView();
                ChonCot frm = new ChonCot();
                frm.CreateDataset();
                for (int i = 0; i < dataGridView1.Columns.Count; i++)
                {
                ...
                frm.ShowDialog();
                query = " select " + Check100() + LayTenCot(Global.listColumns) + " from tblTongHopThongTin " + ShowDieuKien();
                LoadGrid(query);               
        ...
        private void btnSaveFilterClick(object sender, EventArgs e)
        {
                    ...
                    case (2):
                        {
                            ...
                            _hienthi_dieukienloc = txt + " [Từ: " + vt + ", Đến: " + vp + " ] ";
                            Global._dieukien.Add(dieukienloc);
                            Global._hienthi_dieukien.Add(_hienthi_dieukienloc);
                            break;
                        }
                    ...
                    case (3):
                        {
                            ...
                            dieukienloc = col + " >= '" + vt + "' and " + col + " <= '" + vp + "' ";
                            _hienthi_dieukienloc = txt + "[ Từ ngày: " + vt + ", Đến ngày: " + vp + " ] ";
                            Global._dieukien.Add(dieukienloc);
                            Global._hienthi_dieukien.Add(_hienthi_dieukienloc);
                            break;
                        }
            ...
            {
                MessageBox.Show("Bạn đã sai gì đó...\n " + ex.Message);
            }
        ...
        private void _hienthidieukientonghop()
        {
            ...
            if (Global._hienthi_dieukien.Count > 0)
            {
                Global._Notification = Global._hienthi_dieukien[0];
                for (int i = 1; i < Global._hienthi_dieukien.Count; i++)
                {
        ...
        private void AddStyleDataGridView1ColumnHeader()
        {
            ...
            _list2Textbox.Clear();
            _listComboBox.Clear();
            _listDatetimePicker.Clear();
            query = " select * from tblColumns ";      //MaKieuTimKiem
            cls.OpenConnection();
        ...
        private void btnXuatExcel_Click(object sender, EventArgs e)
        {
            ...
            string DefaultPath = System.Windows.Forms.Application.StartupPath + "\\ThongKe";
        ...
        private void cmbTTHS_SelectedIndexChanged(object sender, EventArgs e)
        {
            int _count = Global._dieukien.Count;                
            if (_count > 0)
            {
                for (int i = 0; i < Global._dieukien.Count; i++)
                {
                    if (Global._dieukien[i].Contains("TrangThaiHoSoCapGCN"))
                    {
                        Global._dieukien.Remove(Global._dieukien[i]);
                        Global._hienthi_dieukien.Remove(Global._hienthi_dieukien[i]);
                    }
            ...
            if (cmbTTHS.Text.Trim() != "")
            {

                string _query = " select KyHieu from tblTuDienTrangThaiHoSo where MoTa = N'" + cmbTTHS.Text.Trim() + "'";
                string[] kq = cls._ExecuteReader(_query, "KyHieu");
                Global._MaTrangThaiHoSo = kq[0].ToString();
                Global._dieukien.Add(" TrangThaiHoSoCapGCN = '" + Global._MaTrangThaiHoSo + "' ");
                Global._hienthi_dieukien.Add(" Trạng thái hồ sơ = " + cmbTTHS.Text.Trim());
            }
            else
            {
                Global._MaTrangThaiHoSo = "";
        ...
        private void btnHoSoBienDong_Click(object sender, EventArgs e)
        {
            DanhSachBienDong frm = new DanhSachBienDong();
            frm.Show();
            this.Hide();
        }
