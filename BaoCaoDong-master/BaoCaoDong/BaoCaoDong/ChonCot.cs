...
{
    public partial class ChonCot : Form
    {
        public ChonCot()
        {
            InitializeComponent();
        ...
        {
            string _query = " select * from tblQuanLyBang ";
        ...
        {
            string _query1 = " select count(*) from tblColumns where TrangThai = '1' and Tenbang = '"+TenBang+"'";
        ...
        private void comboBox1_SelectedIndexChanged(object sender, EventArgs e)
        {
            switch (comboBox1.Text.Trim())
            {
                ...
                case "Bảng Hồ sơ cấp Giấy chứng nhận":
                    {
                        dataGridView1.CurrentCell = dataGridView1.Rows[DemSoDongCuaBang("tblThuaDatCapGCN")].Cells[0];
                        break;
                    }
                case "Bảng Hồ sơ phê duyệt":
                    {
                        int _index = DemSoDongCuaBang("tblHoSoCapGCN") + DemSoDongCuaBang("tblThuaDatCapGCN");
                        dataGridView1.CurrentCell = dataGridView1.Rows[_index].Cells[0];
                        break;
                    }
                case "Bảng Hồ sơ thẩm định":
                    {
                        int _index = DemSoDongCuaBang("tblHoSoCapGCN") + DemSoDongCuaBang("tblThuaDatCapGCN")+DemSoDongCuaBang("tblHoSoPheDuyet");
                        dataGridView1.CurrentCell = dataGridView1.Rows[_index].Cells[0];
                        break;
                    }
                case "Bảng Hồ sơ tiếp nhận":
                    {
                        int _index = DemSoDongCuaBang("tblHoSoCapGCN") + DemSoDongCuaBang("tblThuaDatCapGCN") + DemSoDongCuaBang("tblHoSoPheDuyet")+DemSoDongCuaBang("tblHoSoThamDinh");
                        dataGridView1.CurrentCell = dataGridView1.Rows[_index].Cells[0];
                        break;
                    }
                case "Bảng Thông tin chủ":
                    {
                       
                        dataGridView1.CurrentCell = dataGridView1.Rows[dataGridView1.Rows.Count -6].Cells[0];
                        break;
                    }
            }
        }
